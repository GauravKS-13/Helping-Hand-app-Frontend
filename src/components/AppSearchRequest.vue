      ...
      <div class="request-menu">
        ...
        <section class="search-form">
            ...
            <form class="search-form-details" @submit.prevent ="searchRequestbyTime">
                ...
                <select class="select-class" id="search-by-period" v-model="parameters.timeZone">
        ...
        <div v-else-if="status === 'Loaded'" class="search-results">
            <div class="card-template" v-if="foundRequests.length>0">
                <div class="request-card-container">  
                    <div class="request-card" v-for="request in foundRequests" :key="request._id" :value="request._id">
                        <div class="card-img-container">
                            <img src="../assets/blood-drop.png" alt="requester-logo" class="card-image" height="150px" width="200px">
                        </div>
                        <div class="request-card-details">
                            <p class="name"> <span>Name: </span>{{ request.patientName }}</p>
                            <p class="blood-group"> <span>Blood Group: </span>{{ request.bloodGroup }}</p>
                            <p class="required-units"> <span>Required Units: </span> {{ request.units }}</p>
                            <p class="required-date"> <span>Required Date: </span>{{ formateDate(request.date) }} </p> 
                            <p class="required-condition"> <span>Urgent Requirment: </span>{{ request.urgerntRequirment }} </p>        
                            <button class="more-details" @click="searchRequestById( request ) ">More Details</button>
                            <button class="delete-details" @click="removeRequest( request )">Delete</button>
                        </div>
                    </div>
        ...
        <div class="full-details-container" v-if="showFullDetails">
            <div class="full-details">
                <div class="about-patient">
                    ...
                    <p> <span class="details-tag">Name: </span> {{ requestDetails.patientName }}</p>
                    <p><span class="details-tag">Age: </span> {{requestDetails.age}}</p>
                    <p><span class="details-tag">Gender: </span> {{requestDetails.gender}}</p>
                    <p><span class="details-tag">Blood Group: </span>{{ requestDetails.bloodGroup}}</p>
                    <p><span class="details-tag">Required Units: </span>{{requestDetails.units}} </p>
                    <p><span class="details-tag">Required Date: </span> {{ formateDate(requestDetails.date) }}</p>
                    <p class="details-tag"> <span>Urgent Requirment: </span> {{ requestDetails.urgerntRequirment }}</p> 
                    <p><span class="details-tag">City: </span> {{requestDetails.location.city}} </p>
                    <p><span class="details-tag">State </span> {{ requestDetails.location.state}}</p>
                    <p><span class="details-tag">Email: </span> {{ requestDetails.email}}</p>
                    <p><span class="details-tag">Contact: </span> {{ requestDetails.contact}} </p>
                    <div class="btn-container">
                        <button class="back-button" @click="backToPage()"> Back to Page</button>
                    </div>   
            </div>
        </div> 
