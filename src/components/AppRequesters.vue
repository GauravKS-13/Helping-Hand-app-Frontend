...
<main class="main-class">
    ...
    <div class="request-container" v-if=" status === 'Loaded' ">
        ...
        <!-- <div class="request-card" v-for="requester in allRequesters" :key= "requester._id"> -->
        <div class="request-card-container">  
            <div class="request-card" v-for="requester in allRequesters" :key="requester._id">
                <div class="card-img-container">
                    <img src="../assets/blood-donation.png" alt="requester-logo" class="card-image" height="150px" width="200px">
                </div>
                <div class="request-card-details">
                    <p class="name"> <span>Name:</span> {{requester.patientName}}</p>
                    <p class="blood-group"> <span>Blood Group:</span> {{ requester.bloodGroup}}</p>
                    <p class="required-units"> <span>Required Units:</span> {{requester.units}}</p>
                    <p class="required-date"> <span>Required Date:</span> {{ formateDate(requester.date) }}</p> 
                    <p class="required-condition"> <span>Urgent Requirment:</span> {{ requester.urgerntRequirment }}</p>        
                    <button class="more-details" @click="showDetails( requester )">More Details</button>
                </div>
            </div>
            <div class="request-card dummy-class">
            </div>
        </div>
        ...
        <div class="full-details">
            <div class="about-patient">
                ...
                <p><span class="details-tag">Gender:</span> {{request.gender}}</p>
                 <p><span class="details-tag">Blood Group:</span> {{ request.bloodGroup}}</p>
                <p><span class="details-tag">Required Units:</span> {{request.units}} </p>
                <p><span class="details-tag">Required Date:</span> {{ formateDate(request.date) }}</p>
                 <p class="details-tag"> <span>Urgent Requirment:</span> {{ request.urgerntRequirment }}</p> 
                <p><span class="details-tag">City:</span> {{request.location.city}} </p>
                <p><span class="details-tag">State</span> {{ request.location.state}}</p>
